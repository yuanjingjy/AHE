...
for i=1:length(FileList)
   ...
   if (FileList(i).isdir==1 && filename_i(1)=='s' )
     ...
     for j=1:length(file_tmp)%对于每一个hea文件
      ...
      %===========================判断是否发生AHE=========================%
      [ AHEdata,INI,INI0,len,AHE_episode] = findAHE( val_final(:,4),60,21,62,0.9);
      AHEname=[filename_tmp(1:end-4),'_AHE.mat'];
      if length(AHE_episode)>0
          AHE_tmp=val_final(AHE_episode(1):AHE_episode(2),:);
          save(['M:\AHE\',AHEname],'AHE_tmp');
      end
