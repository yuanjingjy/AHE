...
(
  ...
  yes.intime,
  ye24.starttime
  FROM  yj_eigen_sec yes
    RIGHT JOIN yj_eigen_24 ye24
ON yes.subject_id=ye24.subject_id
AND yes.icustay_id=ye24.icustay_id
AND yes.hadm_id=ye24.hadm_id
)
SELECT
tp.*,
  (date_part('epoch'::text, (tp.starttime - tp.intime)) / (((60 * 60) * 24))::double precision) AS icu_length_of_stay
  FROM timepoint tp
...
SELECT
  ...
  ycs.intime,
  ycs.charttime,
   (date_part('epoch'::text, (ycs.charttime - ycs.intime)) / (((60 * 60) * 24))::double precision) AS length_begin
  FROM  yj_cohort_sec ycs
